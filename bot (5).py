        ...
        try:
            ...
            for role in member.roles:
                if role.name.startswith("Level "):
                    try:
                        ...
                        if 1 <= level_num <= 10:
                            role_xp = LEVEL_THRESHOLDS.get(level_num, 0)
                            level_role_xp = max(level_role_xp, role_xp)
                    except:
                        continue
            ...
            if guild_id in self.role_xp_assignments:
                ...
                for role in member.roles:
                    if str(role.id) in role_assignments:
                        role_xp = role_assignments[str(role.id)]
                        custom_role_xp += role_xp
            ...
            for role in member.roles:
                ...
                # Streak roles give 5 XP each  
                    ...
                    auto_role_xp += 5
                    streak_roles_found.append(role.name)
            
            # Calculate total XP - use sum of quest XP + all role bonuses
            if level_role_xp > 0:
                total_xp = max(level_role_xp, base_xp) + custom_role_xp + auto_role_xp
            else:
                total_xp = base_xp + custom_role_xp + auto_role_xp
            
            # Log badge/streak roles found for debugging
            if badge_roles_found or streak_roles_found:
                print(f"Role XP for {member.display_name}: Badge roles: {badge_roles_found}, Streak roles: {streak_roles_found}, Auto XP: {auto_role_xp}")
...
async def assign_badge_xp(ctx, xp_amount: int, *roles: discord.Role):
    ...
    # If roles are provided, use those; otherwise auto-detect badge roles
    if roles:
        ...
        for role in ctx.guild.roles:
            if "badge" in role.name.lower():
                badge_roles.append(role)
        detection_mode = "auto"
    
    if not badge_roles:
        if detection_mode == "auto":
            embed = discord.Embed(
                title="❌ No Badge Roles Found",
                description="No roles with 'badge' in the name were found in this server.\n\n**Manual Selection:** `-assignbadgeXP 5 @role1 @role2`",
                color=0xff0000
            )
        else:
            embed = discord.Embed(
                title="❌ No Roles Specified",
                description="No valid roles were provided for XP assignment.",
                color=0xff0000
            )
        await ctx.send(embed=embed)
        return
    ...
    for role in badge_roles:
        ...
        if role_id_str not in role_assignments:
            ...
            assigned_count += 1
            role_list += f"• **{role.name}** - {xp_amount} XP\n"
        else:
            current_xp = role_assignments[role_id_str]
            role_list += f"• **{role.name}** - Already assigned {current_xp} XP (skipped)\n"
...
async def assign_streak_xp(ctx, xp_amount: int, *roles: discord.Role):
    ...
    # If roles are provided, use those; otherwise auto-detect streak roles
    if roles:
        ...
        streak_roles = []
        for role in ctx.guild.roles:
            if "streak" in role.name.lower():
                streak_roles.append(role)
        detection_mode = "auto"
    
    if not streak_roles:
        if detection_mode == "auto":
            embed = discord.Embed(
                title="❌ No Streak Roles Found",
                description="No roles with 'streak' in the name were found in this server.\n\n**Manual Selection:** `-assignstreakXP 10 @role1 @role2`",
                color=0xff0000
            )
        else:
            embed = discord.Embed(
...
@commands.has_permissions(manage_roles=True)
async def unassign_role_xp(ctx, role: discord.Role):
    """Remove XP assignment from a role (staff only)"""
    guild_id = ctx.guild.id
    ...
    if guild_id not in quest_bot.role_xp_assignments:
        embed = discord.Embed(
            ...
            description=f"Role **{role.name}** has no XP assignment to remove.",
            color=0xff9900
        )
        await ctx.send(embed=embed)
        return
    ...
    old_xp = role_assignments[role_id_str]
    del role_assignments[role_id_str]
    quest_bot.save_settings(guild_id)
    
    embed = discord.Embed(
        title="Role XP Unassigned",
        description=f"Removed XP assignment from role **{role.name}** (was {old_xp} XP)",
        color=0x00ff00
    )
    await ctx.send(embed=embed)
...
async def check_role_xp(ctx, role: discord.Role):
    ...
    xp_amount = role_assignments[role_id_str]
    embed = discord.Embed(
        title="🔍 Role XP Check",
...
async def all_quests(ctx):
    ...
    try:
        ...
        for i, (title, message_id, channel_id) in enumerate(quests, 1):
            ...
            quest_list.append(f"**{i}.** {title} (in {channel_mention})")
    ...
    try:
        ...
        try:
            ...
            if guild_member:
                ...
                # Auto-detected badge and streak roles (tracked separately)
                for role in guild_member.roles:
                    role_name_lower = role.name.lower()
                    if "badge" in role_name_lower:
                        badge_xp += 5
...
async def show_commands(ctx):
    ...
    **XP Management:**
    ...
    `-assignbadgeXP <amount> [@role1] [@role2]...` - Assign XP to badge roles (auto-detects or specify roles)
    `-assignstreakXP <amount> [@role1] [@role2]...` - Assign XP to streak roles (auto-detects or specify roles)
    `-unassignroleXP <role>` - Remove XP assignment from role
